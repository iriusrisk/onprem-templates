...
- **Certificates**: Self-signed certs are generated automatically if not provided.  
  Place your `cert.pem`, `key.pem`, and `ec_private.pem` in `container-compose/` if using custom ones. For production environments, it is **highly recommended** to provide your own securely signed certificates.
